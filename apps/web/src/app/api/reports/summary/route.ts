...
export async function GET(request: NextRequest) {
  ...
  try {
    const [adherence, mood, mealHydration, incidents, timeline] = await Promise.all([
      getAdherence(actor, parsed.data),
      getMoodTrends(actor, parsed.data),
      getMealHydrationSummary(actor, parsed.data),
      getIncidents(actor, parsed.data),
      getTimeline(actor, parsed.data),
    ]);
